...
# Load base WR/TE stats
rec_stats <- load_player_stats(
  ...
  select(-contains("passing"), -contains("pass"), -attempts, -completions,
         ...
         -contains("fumbles"), -contains("rush"), -carries)
...
# Pull Active WR/TEs
active_recs <- load_rosters_weekly(2017:most_recent_season(F)) %>%
  filter(position %in% c("WR", "TE")) %>%
  filter(season == most_recent_season(F), status == "ACT") %>% # Filter active QBs
  pull(full_name)

nextgen_rec <- load_nextgen_stats(2017:most_recent_season(F), stat_type = "receiving") %>%
  select(-player_first_name, -player_last_name, -player_jersey_number, -player_short_name,
         -contains("expected"), -player_position, -rec_touchdowns, -receptions, -targets,
         -yards, -player_gsis_id) %>%
...
rec_stats <- rec_stats %>%
  left_join(nextgen_rec, by = c("recent_team", "player_display_name", "season", 
                                  "season_type", "week")) %>%
  ...
  left_join(adv_rec, by = c("player_display_name", "recent_team", "season", "week", "game_id",
                            "opponent_team")) %>%
  filter(player_display_name %in% active_recs)
...
rec_stats <- rec_stats %>%
  ...
  # Rearrange columns
  select(-headshot_url, -position_group, -player_name, -home_team, 
         -away_team, -player_id) %>%
  # Make rushing_epa NAs 0
  mutate(across(player_display_name, clean_player_names)) %>%
  select(player_display_name, position, recent_team, season, week, game_date, game_id,
         opponent_team, season_type, home_away, everything()) %>%
  filter(targets > 0)

colSums(is.na(rec_stats)) # Check remaining NAs

rm(active_recs, nextgen_rec, adv_rec, game_info)
gc()
```
...
opponent_defense_stats <- rec_stats %>%
...
arrow::write_parquet(gamelogs, "rec_gamelogs_pre.parquet")

rm(rec_stats, defense, adv_def, opponent_defense_stats, cl)
gc()
```
...
```{r}
gamelogs <- arrow::read_parquet("rec_gamelogs_pre.parquet")

gamelogs <- gamelogs %>%
  arrange(season, game_date, week) %>%  # Ensure correct order for rolling calculations
  group_by(player_display_name) %>%
  mutate(across(where(is.numeric) & !starts_with("def") & !all_of(c("season", "week")), 
                ~ lag(rollapply(.x, width=9, 
                  FUN=function(x) {
  ...
  mutate(across(starts_with("def_"), 
                ...
                .names = "roll_{.col}")) %>%
  ungroup() %>%
  select(player_display_name, position, recent_team, season, week, season_type, 
         game_id, game_date, home_away, opponent_team, receptions, receiving_yards,
         receiving_tds, fantasy_points, fantasy_points_ppr, starts_with("roll_")) %>%
...
arrow::write_parquet(gamelogs, "rec_gamelogs_post.parquet")
```

## Modeling - Classification
```{r}
set.seed(6341) # Set seed for reproducibility
gamelogs <- arrow::read_parquet("rec_gamelogs_post.parquet") %>%
  mutate(
    receptions_bin         = cut(receptions, breaks = c(-Inf, 2, 5, 8, Inf), labels = 0:3, right = FALSE),
    receiving_yards_bin    = cut(receiving_yards, breaks = c(-Inf, 25, 50, 75, Inf), labels = 0:3, right = FALSE),
    receiving_tds_bin      = cut(receiving_tds, breaks = c(0, 1, 2, Inf), labels = 0:2, right = FALSE),
    fantasy_points_bin     = cut(fantasy_points, breaks = c(-Inf, 8, 15, 25, Inf), labels = 0:3, right = FALSE),
...
reg_target_vars <- c("receptions", "receiving_yards", "receiving_tds", "fantasy_points",
                 "fantasy_points_ppr")

target_class_vars <- c("receptions_bin", "receiving_yards_bin", "receiving_tds_bin", "fantasy_points_bin",
                 "fantasy_points_ppr_bin")
...
saveRDS(all_models, "prob_model_rec.rds")
```
### Model Evaluation
...
metrics_df
arrow::write_parquet(metrics_df, "prob_metrics_rec.parquet")
```
### Add Class Probabilities
```{r}
bin_breaks_list <- list(
  receptions_bin         = c(-Inf, 2, 5, 8, Inf),
  receiving_yards_bin    = c(-Inf, 25, 50, 75, Inf),
  receiving_tds_bin      = c(0, 1, 2, Inf),
  fantasy_points_bin     = c(-Inf, 8, 15, 25, Inf),
...
arrow::write_parquet(gamelogs, "rec_gamelogs_post_class.parquet")
gc()
```
...
```{r}
set.seed(6341) # Set seed for reproducibility
gamelogs <- arrow::read_parquet("rec_gamelogs_post_class.parquet")
...
target_vars <- c("receptions", "receiving_yards", "receiving_tds", "fantasy_points",
                 "fantasy_points_ppr")
...
saveRDS(final_fit, "WR_TE_Pred_Model.rds")
```
...
metrics_df
coef(final_fit)
arrow::write_parquet(metrics_df, "WR_TE_Metrics.parquet")
```
...
arrow::write_parquet(pred_output, "WR_TE_Preds.parquet")
```
