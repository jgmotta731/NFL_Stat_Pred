...
## Clustering
```{r}
gamelogs <- arrow::read_parquet("qb_gamelogs_pre.parquet") # Efficient Loading

season_data <- load_pfr_advstats(2018:most_recent_season(F), stat_type = "pass", 
                                 summary_level = "season") %>%
  mutate(across(player, clean_player_names)) %>%
  filter(player %in% gamelogs$player_display_name) %>%
  arrange(player, season) %>%  # ensure time order
  group_by(player) %>%
...
player_summary <- gamelogs %>%
  ...
  summarise(across(where(is.numeric), mean, na.rm = TRUE), .groups = "drop") %>%
  left_join(season_data, by = c("player_display_name"="player")) %>%
  drop_na()
...
set.seed(6341)
kmeans_fit <- kmeans(gamelogs_pca, centers = 7, nstart = 100)

# Visualize Clusters with PCA
fviz_cluster(kmeans_fit, data = gamelogs_pca, geom = "point", ellipse.type = "convex") +
  ggtitle("K-Means Clustering of NFL QBs (k = 7)")
...
rm(player_clustered, player_summary, player_scaled, kmeans_fit, res_pca,
   clusters, gamelogs_pca, season_data)
gc()
```
...
gamelogs <- gamelogs %>%
  ...
  select(player_display_name, recent_team, season, week, season_type, 
         ...
         fantasy_points, starts_with("roll_"), cluster) %>%
...
gamelogs <- arrow::read_parquet("qb_gamelogs_post.parquet") # Efficient Loading
